...
static uint8_t x7 = 1;
static uint8_t x8 = 1;
static uint8_t x11 = 1;
static uint8_t x12 = 1;
...
static uint8_t n2=0;
static uint8_t n3=0;
static uint8_t xx2 = 0;
static uint8_t xy2 = 0;
static uint8_t xb ;
static uint8_t xa ;
static uint8_t xp ;
static uint8_t xm ;
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.begin(9600);     // Se inicia la comunicacion serial con un BaudRate de 9600
  while (!Serial) {
    ; // wait for serial port to connect. Needed for Leonardo only
  }
  SPI.setModule(0);         // se inicia el modulo SPI
  Serial.print("Initializing SD card..."); //Inicialización de la SD
  pinMode(10, OUTPUT);
  if (!SD.begin(4)) {
    Serial.println("initialization failed!");   // Si Falla...
    return;
  }
  ...
  empezarjuego();
  String palabra;//concatena los caracter del archivo txt
  char caracter;
  char numero [5];
  ...
  myFile = SD.open("Link.txt");
    if (myFile) {
      ...
      Serial.println("error Sonic.txt");
    }
  ...
  while (!Start) {
    ...
    bottomestado3 = digitalRead(buttonPin3);
    if (bottomestado == LOW) {
      Start = true;
      xp=1;
      xm=0;
...
main_game:
 ...
 bottomestado2 = digitalRead(buttonPin2);
 bottomestado3 = digitalRead(buttonPin2);   
 ...
 if(n1>=3){
  ...
  if (cubod4 < 320){
    ...
    if( shut>=cubod5 && y==1 ){
      FillRect(cubod5, 190, 34, 24, 0x421b);
      x11=0;   
    }
    if(x11==1)
if (cubod5 < 319) {
    LCD_Sprite(cubod5, 190, 34, 23, enemigo1, 3, animsp, 0, 0);
    FillRect(cubod5+34, 190, 34, 24, 0x421b);
}
...
//------------------------------------------------------------------
 ...
 if ((cubox2 >= cuboenex7) && (cubox1 <= cuboenex8) && (yB <= cuboeney3)&&(x3==1)) {
  ...
  if ((cubox2 >= cuboenex9) && (cubox1 <= cuboenex10) && (cuboy2 >= cuboeney1)&&(n1>=3)) {
    gameover();
    }
   if ((cubox2 >= cuboenex11) && (cubox1 <= cuboenex12) && (yB <= cuboeney3)&&(x5==1)&&(n1>=3)) {
    gameover();
    }
     if ((cubox2 >= cuboenex7) && (cubox1 <= cuboenex8) && (yB <= cuboeney3)&&(x6==1)&&(n1>=3)) {
    gameover();
     }
     if ((cubox2 >= cuboenex11+100) && (cubox1 <= cuboenex12+100) && (cuboy2 >= cuboeney1)&&(x5==1)&&(n1>=3)) {
    gameover();
    }
    if ((cubox2 >= cuboenex7) && (cubox1 <= cuboenex8) && (yB <= cuboeney3)&&(x8==1)&&(n2>=3)) {
    gameover();
     }
     if ((cubox2 >= cuboenex11+100) && (cubox1 <= cuboenex12+100) && (cuboy2 >= cuboeney1)&&(x7==1)&&(n2>=3)) {
    gameover();
    }
